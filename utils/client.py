...
# 1. Definimos a Base URL de forma dinâmica. 
# Se não for passada, mantemos a da OpenAI como fallback por segurança.
# Para o Venice, o usuário configurará: https://api.venice.ai/api/v1
JARVINA_BASE_URL = os.getenv("JARVINA_BASE_URL", "https://api.openai.com/v1")

# 2. Lógica de fallback de chaves de API.
# Prioriza a do Jarvina, mas aceita a do Venice ou da OpenAI para facilitar a transição.
JARVINA_API_KEY = (
...
# 3. Configuração do modelo de visão
JARVINA_VISION_MODEL = os.getenv("JARVINA_VISION_MODEL", "qwen-2.5-vl")

# 3. Validação Fail-Fast (Falha Rápida)
if not JARVINA_API_KEY:
    msg = "ERRO: Nenhuma chave de API configurada. Defina JARVINA_API_KEY no ambiente."
    logger.error(msg)
    raise ValueError(msg)

# 4. Instanciação do Cliente Único (Singleton pattern simples)
# Este é o 'client' que todas as ferramentas do MCP vão importar e usar.
jarvina_client = OpenAI(
    api_key=JARVINA_API_KEY,
    base_url=JARVINA_BASE_URL
)

logger.info(f"Jarvina Client inicializado apontando para: {JARVINA_BASE_URL}")
